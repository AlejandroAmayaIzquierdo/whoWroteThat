...
export const createUser = async (provider: Lucia.Providers,providerUserId: string,password: string): Promise<Lucia.User> => {
...
export const getUser = async(provider: Lucia.Providers,providerUserId: string,password: string ): Promise<Lucia.User> => {
    const auth = AuthManager.getInstance().getAuth();
    if(!auth)
        throw { message: "Internal server error", code: 500 };
