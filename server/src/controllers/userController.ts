...
import { SECRET_CLIENT_ID, SECRET_CLIENT_SECRET, GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET,TWITCH_CLIENT_ID,TWITCH_CLIENT_SECRET } from "../index.js";
...
export const oauthTwitchController = async (req: Request, res: Response) => {
try {
        ...
        if (!userInfoResponse.ok) {
            throw { message: "Error while fetching user information from Twitch API", code: userInfoResponse.status };
        }
        ...
        const existingUser = await Db.getInstance().query(
            ...
            FROM user_key
            WHERE twitch_id = '${userInfo.id}'`) as { user_key_id: string,user_id:string,twitch_id: string }[];
        ...
        const resp = await Db.getInstance().query(`UPDATE user_key SET twitch_auth = '${JSON.stringify(tokens)}', twitch_id = '${userInfo.id}' WHERE user_id = '${user.userId}'`);
        console.log(resp);
        const session = await createSession(user.userId);
        return session;
} catch (err) {
